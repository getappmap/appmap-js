...
it('secret in log file', async () => {
  ...
  expect(findings).toHaveLength(2);
  {
    ...
    expect(finding.description).toEqual(
      `A [log event]({logEvent as url}) contains the message {value}. This message contains the secret value {pattern}. This secret is generated by the function [{generatorEvent as prettyName}]({generatorEvent as url}). Later in request processing, the secret is writen to the log.`
    );
  }
  {
