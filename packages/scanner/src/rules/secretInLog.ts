...
const findInLog = (event: Event): MatchResult[] | undefined => {
  ...
  if (matches.length > 0) {
    return matches.map((match) => {
      ...
      let description = `A [log event]({logEvent as url}) contains the message {value}.`;
      if (pattern instanceof RegExp) {
        volatileData.pattern = pattern;
        description += ` This message matches the regular expression {pattern as regexp}, which is likely to be a secret value.`;
      } else {
        volatileData.secret = pattern;
        description += ` This message contains the secret value {pattern}.`;
        if (match.generatorEvent) {
          description += ` This secret is generated by the function [{generatorEvent as prettyName}]({generatorEvent as url}).`;
        }
        description += ` Later in request processing, the secret is writen to the log.`;
      }
      return {
        event,
