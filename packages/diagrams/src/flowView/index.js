...
import { lazyPanToElement, panToNode } from '../util';
...
export default class FlowView extends EventSource {
  constructor(container, options = {}) {
    ...
    this.nodeGroup = select(this.element).append('ul').attr('id', 'nodes');

    this.linkGroup = this.svg.append('g').attr('id', 'links');

    this.valuePopper = select(this.element)
      .append('div')
  ...
  render() {
    ...
    this.nodeGroup.datum(hierarchy(rootEvent)).call(bind);
