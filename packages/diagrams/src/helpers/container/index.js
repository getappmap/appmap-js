...
export default class Container extends EventSource {
  constructor(parent, options = {}, element = null, contentElement = null) {
    ...
    if (parent) {
      parentElement = select(parent).node();
    ...
    if (this.options.zoom) {
      ...
      if (this.options.pan.momentum) {
        momentum(this.zoom, select(this.element));
      }

      select(this.element).call(this.zoom).on('dblclick.zoom', null);
    }
  }
  ...
  centerX(verticalPadding = 0) {
    ...
    const transformMatrix = zoomIdentity.translate(
  ...
  scaleTo(k) {
    select(this.element).transition().duration(100).call(this.zoom.scaleTo, k);
  }
  ...
  get transform() {
    return zoomTransform(this.element);
  }

  set transform(transform) {
    select(this.element).call(this.zoom.transform, transform);
  }
}
