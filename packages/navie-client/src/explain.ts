...
import collectHelp from './help';
import { getLLMConfiguration } from './llmConfiguration';
import detectAIEnvVar from './aiEnvVar';
...
export class Explain extends EventEmitter {
  ...
  async explain(navie: INavie): Promise<void> {
    navie.on('ack', (userMessageId: string, threadId: string) => {
    ...
    navie.on('token', (token: string, _messageId: string) => {
