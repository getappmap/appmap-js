...
describe('Thread', () => {
  ...
  describe('sendMessage', () => {
    it('has context mutated by `@review`', async () => {
      ...
      const context: NavieRpc.V1.UserContext.ContextItem[] = [{ type: 'static', content: 'test' }];
  ...
  describe('addMessageAttachment', () => {
    it('emits an `add-message-attachment` event', () => {
      const listener = jest.fn();
      thread.on('event', 'test-client', listener);
      const attachmentId = thread.addMessageAttachment('test-uri', 'test-content');
      expect(listener).toHaveBeenCalledWith({
    ...
    it('returns the attachment id', () => {
      const result = thread.addMessageAttachment('test-uri', 'test-content');
      const [event] = thread.log;
      expect(result).toBe((event as NavieAddMessageAttachmentEvent).attachmentId);
    });
  });
  ...
  describe('getMessageAttachments', () => {
    it('returns an attachment that was added since the last user message', () => {
      ...
      expect(thread.getMessageAttachments()).toStrictEqual([
        { attachmentId, uri, content, time: expect.any(Number), type: 'add-message-attachment' },
      ]);
    });

    it('ignores attachments that were added before the last user message', () => {
      thread.addMessageAttachment('test-attachment-id', 'test-uri');
      thread.logEvent({ type: 'message', role: 'user', content: 'test', messageId: '1' });
      expect(thread.getMessageAttachments()).toStrictEqual([]);
    });

    it('ignores attachments that were removed', () => {
      const attachmentId = thread.addMessageAttachment('test-attachment-id', 'test-uri');
      thread.removeMessageAttachment(attachmentId);
      expect(thread.getMessageAttachments()).toStrictEqual([]);
    });
  });
