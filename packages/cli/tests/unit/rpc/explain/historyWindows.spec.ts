...
import HistoryWindows from '../../../../src/rpc/explain/navie/historyWindows';
import { tmpdir } from 'os';
import { join } from 'path';
import Thread from '../../../../src/rpc/explain/navie/thread';

// This class has full public access to members of HistoryWindows.
type HistoryWindowsTest = Omit<
  HistoryWindows,
  'getOrCreateThread' | 'writeThread' | 'threads' | 'debounceTimers'
> & {
  debounceTimers: typeof HistoryWindows.prototype['debounceTimers'];
  getOrCreateThread: typeof HistoryWindows.prototype['getOrCreateThread'];
  writeThread: typeof HistoryWindows.prototype['writeThread'];
};

const HistoryStatic = HistoryWindows as unknown as {
...
describe('HistoryWindows', () => {
  let history: HistoryWindowsTest;
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'appmap-test-'));
    history = new HistoryWindows(tmpDir) as unknown as HistoryWindowsTest;
  ...
  describe('load', () => {
    ...
    it('retrieves a thread from memory if it is already loaded', async () => {
      const threadId = '123';
      HistoryStatic.THREADS.set(threadId, new Thread(threadId, 0, ['/home/user']));
