...
import LocalNavie from '../explain/navie/navie-local';
...
async function getWelcomeMessage(navieProvider: INavieProvider): Promise<string> {
  // Case 1: Local Navie providers will continue to have a static welcome message.
  const navie = navieProvider(NOP, NOP, NOP);
  if (navie instanceof LocalNavie) {
