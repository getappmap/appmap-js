...
export const builder = async (args: yargs.Argv) => {
  // ChangeReporter pulls in @appland/models, openapi-diff, sql-formatter and handlebars,
  // so load it only when this command's builder actually runs.
  const { SECTIONS, EXPERIMENTAL_SECTIONS } = await import('./ChangeReporter');
...
export const handler = lazyHandler(() => import('./compareReportHandler'));
