...
import { ConfigOpenApi, Exclusion, ExclusionMatch, match } from '../config/openApi';
...
export default {
  ...
  async handler(argv: Arguments | any) {
    ...
    const config = appmapConfig?.openapi as ConfigOpenApi;
    if (config.schemas) template.components.schemas = config.schemas;
    if (config.overrides) applySchemaOverrides(template.paths, config.overrides);
    if (config.ignore) ignorePaths(template.paths, config.ignore);
...
export function ignorePaths(
  ...
  return Object.entries(paths).reduce((result, [route, routeDefinition]) => {
    const allowedMethods = Object.keys(routeDefinition).filter(
      (method) => match(exclusions, route, method) !== ExclusionMatch.Exclude
    );
    ...
    allowedMethods.forEach((method) => {
      result[route][method] = routeDefinition[method];
    });
