<template>
  <section id="features" class="feature-list">
    <div class="feature-list__container">
      <section-heading title="Features" />
      ...
      <div v-if="showDismissDialog !== null" class="feature-list__modal-overlay">
        <div class="feature-list__modal-content">
          <h3 class="feature-list__modal-title">Why are tests not needed?</h3>
          <textarea
            v-model="dismissReasonInput"
            class="feature-list__modal-textarea"
            placeholder="Enter your reason..."
          ></textarea>
          <div class="feature-list__modal-actions">
            <button
              @click="closeDismissDialog"
              class="feature-list__button feature-list__button--secondary"
            >
              Cancel
            </button>
            <button
              @click="submitDismissReason"
...
// In Vue CLI, you might use raw-loader or fetch it.
const featureYamlContent = `
features:
  - text: "Implemented user login and registration"
    hasCoverage: true
    testDetails:
      file: "auth.service.spec.ts"
      location: "L10-L50"
      tests:
        - name: "should allow a user to register"
          status: "pass"
        - name: "should allow a registered user to login"
          status: "pass"
        - name: "should reject login with incorrect password"
          status: "fail"
          message: "AssertionError: expected false to be true"
  - text: "Added a new dashboard widget for sales overview"
    hasCoverage: false
    aiPrompt: "Generate unit tests for a React component 'SalesOverviewWidget' that displays sales data. It receives 'salesData' as a prop."
  - text: "Refactored the payment processing module"
    hasCoverage: true
    testDetails:
      file: "payment.processor.spec.ts"
      location: "L25-L80"
      tests:
        - name: "should process visa payments correctly"
          status: "pass"
        - name: "should handle payment gateway errors"
          status: "pass"
  - text: "Fixed a bug in the report generation service"
    hasCoverage: false
    aiPrompt: "Write tests for a Node.js function 'generateReport(reportParams)' that queries a database and returns a PDF."
`; // Replace with your actual YAML loading mechanism
...
export default Vue.extend({
  ...
  methods: {
    loadFeatures() {
      try {
        const parsedYaml = parse(featureYamlContent) as { features: Feature[] };
        this.features = parsedYaml.features || [];
      } catch (error) {
        console.error('Error parsing features YAML:', error);
    ...
    submitDismissReason() {
      if (this.showDismissDialog !== null && this.dismissReasonInput.trim()) {
        this.dismissedFeatures.push({
          index: this.showDismissDialog,
          reason: this.dismissReasonInput,
        });
        this.$emit('feature-dismiss', this.showDismissDialog); // Emit event
    ...
    handleFixTest(feature: Feature, test: Test) {
      ...
      console.log('Attempting to fix test:', test.name, 'in feature:', feature.text);
