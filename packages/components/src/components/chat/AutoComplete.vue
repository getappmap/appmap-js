  ...
  <v-popper :arrow="false" align="left" placement="top" ref="autocomplete" class="autocomplete">
    <template #content>
      <ul data-cy="autocomplete">
        <li
          v-for="(item, i) in autoCompletions"
          :key="i"
          :class="{ selected: i === selectionIndex }"
          @click.prevent="onSubmitCompletion(item.command)"
          @mouseover.prevent="selectionIndex = i"
        >
          <span class="command">
            <b :data-active="selectionIndex === i">{{ item.command }}</b>
          </span>
...
export default Vue.extend({
  ...
  props: {
    ...
    commands: {
      type: Array as () => NavieRpc.V1.Commands.Response,
      default: () => [
        {
          command: '@explain',
          description: 'This is the default mode. Navie will explain something about your project.',
        },
        {
          command: '@help',
          description:
        ...
        { command: '@generate', description: 'This mode is optimized for code generation.' },
      ],
    },
  },
  ...
  computed: {
    ...
    activeToken(): string | undefined {
      if (!this.caretOffset || !this.input) return undefined;
      ...
      switch (e.key) {
        ...
        case 'Enter': {
          if (this.autoCompletions.length > 0) {
            const { command } = this.autoCompletions[this.selectionIndex];
            this.onSubmitCompletion(command);
          }
