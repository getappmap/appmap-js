...
export default class ProjectInfoService {
  ...
  promptProjectInfo(isArchitecture: boolean, projectInfo: ProjectInfo[]) {
    ...
    if (appmapConfigs.length > 0) {
      this.interactionHistory.addEvent(
        new ContextItemEvent(PromptType.AppMapConfig, dump(appmapConfigs))
      );
    } else {
      this.interactionHistory.addEvent(
    ...
    if (appmapStats.map((stats) => stats.numAppMaps).reduce((a, b) => a + b, 0) > 0) {
      this.interactionHistory.addEvent(
        new ContextItemEvent(PromptType.AppMapStats, dump(appmapStats))
      );
    } else {
      this.interactionHistory.addEvent(
        new ContextItemEvent(PromptType.AppMapStats, 'The project does not contain any AppMaps.')
      );
    }
    ...
    if (codeEditors.length > 0) {
      ...
      this.interactionHistory.addEvent(
        new ContextItemEvent(PromptType.CodeEditor, 'The code editor is not specified.')
      );
    }
  }
}
